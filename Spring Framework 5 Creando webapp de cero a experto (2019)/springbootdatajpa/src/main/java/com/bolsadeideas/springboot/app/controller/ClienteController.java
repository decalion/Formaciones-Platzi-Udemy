...
public class ClienteController {
	...
	public ResponseEntity<Resource> verFoto(@PathVariable String filename){
			...
			try {
				recurso = uploadServices.load(filename);
	...
	public String listar(@RequestParam (name="page", defaultValue="0") int page, Model model,
		...
		if(auth  != null) {
			logger.info("Utilizando de forma estatica SecurityContextHolder.getContext().getAuthentication(): Hola usuario autenticado, tu username es : ".concat(auth.getName()));
	...
	public String crear(Map<String, Object> model) {
		...
		model.put("accion", "Crear nuevo cliente");
	...
	public String editar(@PathVariable(value ="id") Long id, Map<String,Object> model,RedirectAttributes flash) {
		...
		model.put("accion", "Modificar cliente");
	...
	public String guardar(@Valid @ModelAttribute("cliente") Cliente cliente, BindingResult result,Model model, @RequestParam("file") MultipartFile foto, RedirectAttributes flash ,SessionStatus status) {
		...
		if(!foto.isEmpty()) {
			...
			if(cliente.getId() !=null && cliente.getId() > 0 && cliente.getFoto()!=null && cliente.getFoto().length() >0) {
				...
				uploadServices.delete(cliente.getFoto());
			}
			
			
			String uniqueFileName = null;
			try {
				uniqueFileName = uploadServices.copy(foto);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		//Para eliminar la session una vez guardado el objeto
		status.setComplete();
	...
	public String eliminar(@PathVariable(value ="id") Long id,RedirectAttributes flash) {
		...
		if(id > 0) {
			...
			flash.addFlashAttribute("success", "Cliente Eliminado correctamente");
				...
				if(uploadServices.delete(cliente.getFoto())) {
